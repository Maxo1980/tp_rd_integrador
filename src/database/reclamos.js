...
export default class Reclamos {
  ...
  // *** paginacion ***
  buscarTodosConPaginacion = async (page = 1, limit = 10) => {
    //page era el numero de paginas y limit es el numero de pagina que me da por resultados
    const offset = (page - 1) * limit; // Calculo con offset desde que registo me devuelve el reusltado, y limit es cuanto registro me da
    const sql = `
            SELECT 
                r.idReclamo, 
                r.asunto, 
            ...
            INNER JOIN usuarios AS u ON u.idUsuario = r.idUsuarioCreador
            LIMIT ? OFFSET ?; 
        `; // agregue limit y offet en la consulta sql
  ...
  buscarDatosReportePdf = async () => {
    ...
    const datosReporte = {
      ...
      descripcionTipoRreclamoFrecuente:
        result[3][0].descripcionTipoRreclamoFrecuente,
